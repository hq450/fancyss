...
start_socks5(){
	...
	if [ "$ss_local_obfs" == "0" ];then
		...
		ss-local -b :: -s "$ss_local_server" -p "$ss_local_port" -l "$ss_local_proxyport" -k "$ss_local_password" -m "$ss_local_method" -u $ARG_ACL --plugin obfs-local --plugin-opts "$ARG_OBFS" -f /var/run/ss_local.pid
	fi
	echo_date 完成...
}
