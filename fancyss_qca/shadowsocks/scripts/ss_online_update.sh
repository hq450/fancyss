...
get_ssr_node_info(){
	decode_link="$1"
	action="$2"
	server=$(echo "$decode_link" | awk -F':' '{print $1}' | sed 's/ //g')
	...
	password=$(echo $password | base64_encode | sed 's/ //g')
	...
	[ -n "$protoparam_temp" ] && protoparam=$(decode_url_link $protoparam_temp | sed 's/_compatible//g' | sed 's/ //g') || protoparam=''
...
del_none_exist(){
	...
	local DEL_INDEXS=$(echo $DIFF_SERVERS $DIFF_REMARKS | sed 's/ /\n/g' | sort | uniq -d)
	# 删除操作
